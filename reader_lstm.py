...
class Data(object):
    ...
    def load(self):
        ...
        self.issues=to_categorical(list(df["issues"]), num_classes=13)
    ...
    def transform(self):
        ...
        self.issues=np.array(self.issues)
    ...
    def generator(self, batch_size):
        ...
        while True:
            try:
                ...
                yield ([np.array(self.inputs[batch_ids], dtype=int),np.array(self.targets[batch_ids],dtype=int),np.array(self.issues[batch_ids])],np.array(self.labels[batch_ids]))
...
if __name__ == '__main__':
    ...
    ds = Data('./data/train_lstm.csv', vocab,kb_vocabulary)
    ds.kb_out()
    g = ds.generator(32)
    ds.load()
    ds.transform()
